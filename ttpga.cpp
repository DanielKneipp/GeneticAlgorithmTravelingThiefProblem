                 ...
                 unsigned numGenerations )
{
    ...
    for( unsigned i = 0; i < numGenerations; i++ )
    {
        ...
        //** Selection Step **//
        this->population = SelectionMethod::tournament( this->population );
...
std::vector< TTPIndividual > TTPGA::generatePopulation( unsigned numIndividuals )
{
    ...
    for( unsigned i = 0; i < numIndividuals; i++ )
    {
        ...
        individual.features.tour = std::random_shuffle( baseTour.begin() + 1, baseTour.end() - 1 );
        ...
        {
            individual.features.pickingPlan.push_back( GeneticUtils::genIntRandNumber< unsigned char >( 0, 1 ) );
        }
